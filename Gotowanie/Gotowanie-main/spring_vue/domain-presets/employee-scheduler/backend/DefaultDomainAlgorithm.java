 ...
 * {@code hours Ă— hourlyRate Ă— shiftTypeMultiplier} (EVENING Ă—1.1).</p>
 *
...
public class DefaultDomainAlgorithm implements DomainAlgorithm {
    ...
    public DomainAlgorithmResult evaluate(DomainAlgorithmInput input) {
        ...
        if (resource.getStatus() != ResourceStatus.ACTIVE) {
            breakdown.addRule("RESOURCE_CHECK: pracownik niedostÄ™pny (" + resource.getStatus() + ")");
            return DomainAlgorithmResult.failure(List.of("Pracownik nie jest dostÄ™pny"), breakdown.build());
        }
        breakdown.addRule("RESOURCE_CHECK: ok");

        if (input.startAt() == null || input.endAt() == null || !input.startAt().isBefore(input.endAt())) {
            breakdown.addRule("TIME_RANGE_CHECK: nieprawidĹ‚owy zakres");
        ...
        if (!collisions.isEmpty()) {
            errors.add("Zmiana koliduje z istniejÄ…cymi wpisami grafiku: " + ids(collisions));
        ...
        breakdown.addLine("baseCost", baseCost,
                workedHours.toPlainString() + "h Ă— " + hourlyRate.toPlainString() + " PLN/h");
        breakdown.addRule("BASE_COST: " + workedHours.toPlainString() + "h Ă— " + hourlyRate.toPlainString());
        ...
        if (surcharge.signum() != 0) {
            breakdown.addLine("shiftTypeSurcharge", surcharge,
                    "zmiana " + shiftType + " Ă— " + shiftMultiplier.toPlainString());
        }
        breakdown.addRule("SHIFT_TYPE_MULTIPLIER: Ă— " + shiftMultiplier.toPlainString() + " (" + shiftType + ")");
    ...
    private DomainAlgorithmResult evaluateGeneric(DomainAlgorithmInput input) {
        ...
        if (resource == null) {
            ...
            return DomainAlgorithmResult.failure(List.of("ZasĂłb jest wymagany"), breakdown.build());
        }
        if (resource.getStatus() != ResourceStatus.ACTIVE) {
            breakdown.addRule("RESOURCE_CHECK: zasĂłb nieaktywny (" + resource.getStatus() + ")");
            return DomainAlgorithmResult.failure(List.of("ZasĂłb nie jest aktywny"), breakdown.build());
        }
        breakdown.addRule("RESOURCE_CHECK: ok");
        ...
        if (profile.algorithmMode().checksTime() || hasStart || hasEnd) {
            if (hasStart ^ hasEnd) {
                errors.add("Zakres dat jest niekompletny (wymagane poczÄ…tek i koniec)");
                breakdown.addRule("TIME_RANGE_CHECK: niekompletny zakres");
            } else if (hasStart) {
                if (!input.startAt().isBefore(input.endAt())) {
                    errors.add("NieprawidĹ‚owy zakres dat: poczÄ…tek musi byÄ‡ przed koĹ„cem");
                    breakdown.addRule("TIME_RANGE_CHECK: nieprawidĹ‚owy zakres");
                } else {
                    ...
                    if (profile.algorithmMode().checksTime()) {
                        List<RequestEntity> collisions =
                                collisionDetector.findCollisions(input.startAt(), input.endAt(), activeExisting);
                        if (!collisions.isEmpty()) {
                            errors.add("Termin koliduje z istniejÄ…cymi zgĹ‚oszeniami: " + ids(collisions));
                            breakdown.addRule("TIME_COLLISION_CHECK: kolizja (" + collisions.size() + ")");
                            breakdown.addNote("Kolizja z zgĹ‚oszeniami: " + ids(collisions));
                        } else {
        ...
        if (input.quantity() != null && input.quantity() <= 0) {
            errors.add("IloĹ›Ä‡ musi byÄ‡ dodatnia");
            breakdown.addRule("QUANTITY_CHECK: nieprawidĹ‚owa iloĹ›Ä‡");
        } else if (profile.requiresQuantity() && input.quantity() == null) {
            errors.add("Wymagana jest iloĹ›Ä‡");
            breakdown.addRule("QUANTITY_CHECK: brak wymaganej iloĹ›ci");
        }
        ...
        if (profile.algorithmMode().checksCapacity()
            ...
            if (!capacityMatcher.fits(capacity, used, qty)) {
                errors.add("Przekroczono pojemnoĹ›Ä‡: uĹĽyte %d/%d, ĹĽÄ…dane %d".formatted(used, capacity, qty));
    ...
    private BigDecimal computeGenericValue(ResourceEntity resource, Map<String, Object> requestMetadata,
        ...
        breakdown.addLine("WartoĹ›Ä‡ bazowa", subtotal,
                "%s Ă— %d (%s) Ă— %d".formatted(base.toPlainString(), durationUnits, profile.pricingUnit(), qty));
        ...
        if (multiplier != null && multiplier.compareTo(BigDecimal.ONE) != 0) {
            ...
            breakdown.addLine("MnoĹĽnik", delta, "Ă— " + multiplier.toPlainString());
        }
