...
public class DefaultDomainAlgorithm implements DomainAlgorithm {
    ...
    public DomainAlgorithmResult evaluate(DomainAlgorithmInput input) {
        if (isConferenceInput(input)) {
            return evaluateConference(input);
        }
        return evaluateGeneric(input);
    }

    private boolean isConferenceInput(DomainAlgorithmInput input) {
        return hasAny(input.requestMetadata(), "meetingTitle", "attendeeCount")
                || (input.resource() != null && hasAny(input.resource().getMetadata(),
                "roomName", "capacity", "hasProjector", "hasVideoConference", "hourlyRate"));
    }

    private DomainAlgorithmResult evaluateConference(DomainAlgorithmInput input) {
        ...
        if (resource == null) {
            breakdown.addRule("RESOURCE_CHECK: brak sali");
            return DomainAlgorithmResult.failure(List.of("Zasób jest wymagany"), breakdown.build());
        }
        if (resource.getStatus() != ResourceStatus.ACTIVE) {
            breakdown.addRule("RESOURCE_CHECK: sala nieaktywna (" + resource.getStatus() + ")");
            return DomainAlgorithmResult.failure(List.of("Sala nie jest dostępna"), breakdown.build());
        }
        ...
        if (profile.algorithmMode().checksTime() || hasStart || hasEnd) {
            if (hasStart ^ hasEnd) {
                ...
                if (!input.startAt().isBefore(input.endAt())) {
                    ...
                    if (profile.algorithmMode().checksTime()) {
                        ...
                        if (!collisions.isEmpty()) {
                            errors.add("Termin koliduje z istniejącymi rezerwacjami: " + ids(collisions));
                            breakdown.addRule("TIME_COLLISION_CHECK: kolizja (" + collisions.size() + ")");
                            breakdown.addNote("Kolizja z rezerwacjami: " + ids(collisions));
                        } else {
                ...
                errors.add("Wymagany jest zakres czasu rezerwacji");
                breakdown.addRule("TIME_RANGE_CHECK: brak wymaganego zakresu");
            }
        }

        Integer attendeeCount = readInteger(input.requestMetadata(), "attendeeCount");
        if (attendeeCount != null && resource.getCapacityValue() != null) {
            if (attendeeCount > resource.getCapacityValue()) {
                errors.add("Liczba uczestników (%d) przekracza pojemność sali (%d)"
                        .formatted(attendeeCount, resource.getCapacityValue()));
                breakdown.addRule("CAPACITY_CHECK: przekroczono (%d > %d)"
                        .formatted(attendeeCount, resource.getCapacityValue()));
            } else {
                breakdown.addRule("CAPACITY_CHECK: ok (%d <= %d)"
                        .formatted(attendeeCount, resource.getCapacityValue()));
            }
        } else if (attendeeCount == null) {
            errors.add("Liczba uczestników jest wymagana");
            breakdown.addRule("CAPACITY_CHECK: brak danych");
        }

        BigDecimal calculatedValue = null;
        if (profile.algorithmMode().calculatesValue()) {
            calculatedValue = computeConferenceValue(resource, durationUnits, profile, breakdown);
        }

        if (!errors.isEmpty()) {
            return DomainAlgorithmResult.failure(errors, breakdown.build());
        }
        breakdown.addRule("TOTAL_PRICE: " + calculatedValue);
        return DomainAlgorithmResult.success(calculatedValue, resource.getId(), breakdown.build());
    }

    private BigDecimal computeConferenceValue(ResourceEntity resource, long durationUnits,
                                              DomainProfile profile,
                                              AlgorithmBreakdownBuilder breakdown) {
        BigDecimal hourlyRate = resource.getBaseValue() != null ? resource.getBaseValue() : BigDecimal.ZERO;
        BigDecimal total = hourlyRate.multiply(BigDecimal.valueOf(durationUnits));
        breakdown.addLine("hourlyRate", total,
                "%s x %d (%s)".formatted(hourlyRate.toPlainString(), durationUnits, profile.pricingUnit()));
        breakdown.addRule("COST_CALCULATION: %s x %d = %s"
                .formatted(hourlyRate.toPlainString(), durationUnits, total.toPlainString()));
        return total;
    }
